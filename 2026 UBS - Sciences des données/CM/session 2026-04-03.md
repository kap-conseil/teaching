...
**Modèle global :** un seul modèle pour prévoir les rendements de toutes les variétés, tous les producteurs et tous les horizons à prévoir.
...
- 1/ Un par campagne (année) ? (Très) mauvaise idée car il faut un modèle par année, donc pas d'historique d'une année à l'autre
- 2/ Un par producteur ? Mauvaise idée car il y a des nouveaux producteurs sans historique
- 3/ Un par variété ? Pourquoi pas si les variétés ont de l'historique
...
Pour **3/**, testé dans le cas présent avec empriquement une sous-performance des modèles appris par variété versus un modèle global (multi-variété avec regresseur donnant la variété).

Pour **4/**, modèle multi-horizon non-testé : pour la mise en oeuvre il faut construire un dataset d'apprentissage avec les 6 horizons à prévoir en même temps. ➡️  Techniquement plus complexe. 

## Un modèle global Vs des modèles locaux - vu de la recherche

Pour les prévisions de séries chrnologiques, la question de l'apprentissage global (un modèle pour plusieurs séries) versus local (un modèle par série) est un sujet de recherche actif. Papier de référence sur ce sujet :
...
Local vs global : Un modèle global peut reproduire exactement ce qu’un ensemble de modèles locaux ferait. Mais un modèle global peut aussi apprendre des régularités communes à toutes les séries, ce qui peut améliorer la précision de la prévision, surtout pour les séries avec peu de données.

Diversité des séries : les modèles locaux deviennent compliqués quand il y a beaucoup de séries. La complexité de mise en oeuvre augmente aussi.

Mémoire temporelle : pour des modèles autorégressifs, l’approche globale peut utiliser une mémoire plus longue sans pénalité équivalente aux modèles locaux. C’est directement aligné avec tes slides Processus temporel et Non-échangeabilité, où les dépendances entre semaines sont centrales.

Résultats empiriques : même des modèles globaux simples (linéaires) battent souvent des approches locales, parfois avec beaucoup moins de paramètres. Cela soutient ton message Mise en oeuvre : de bonnes performances peuvent venir de modèles plus simples, maintenables et rapides à réentraîner.

Implication opérationnelle pour ton cas maraîcher : commencer par un modèle global rendement avec variables exogènes (météo, variété, producteur, horizon) est cohérent avec l’article, puis comparer à quelques variantes locales en validation temporelle stricte. Ça relie naturellement tes sections Choix de l’algorithme, Validation croisée, et modèle global vs local.

## Validation croisée

Pourquoi c'est indispensable ?
* Permet d'estimer la performance du modèle sur des données non vues, en respectant les propriétés des données
* Permet de comparer différents modèles et de choisir les meilleurs hyperparamètres
* Permet de détecter et de prévenir le surapprentissage (overfitting)
